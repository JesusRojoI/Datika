...
export default function CheckoutPage() {
  ...
  const { formatPrice, currency, convertPrice, usdPerMXN } = useCurrencyStore();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ==========================================
      // CONVERSIÓN DE MONEDA PARA EL PAGO
      // ==========================================
      const montoFinal = convertPrice(getTotal());
      const currencyCode = currency === 'MXN' ? '484' : '840';
      const currencyLabel = currency === 'MXN' ? 'MXN' : 'USD';

      console.log('💰 Procesando pago:');
      console.log('   • Moneda:', currencyLabel);
      console.log('   • Código:', currencyCode);
      console.log('   • Monto:', montoFinal.toFixed(2), currencyLabel);
      console.log('   • Tipo de cambio:', usdPerMXN, 'USD por 1 MXN');
      ...
      const paymentResponse = await fetch('/api/procesar-pago', {
        ...
        body: JSON.stringify({
          ...
          cvv: formData.cvv,
          monto: montoFinal,
          currency: currencyCode, // ← Enviar código de moneda
  ...
  usdPerMXN: usdPerMXN, // ← AGREGAR ESTO
};
