    ...
    <div ref={flyoutRef} className="fixed top-20 right-4 z-50 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 animate-scale-in overflow-hidden" onMouseLeave={() => setFlyoutOpen(false)}>
      ...
      <div className="max-h-64 overflow-y-auto p-4 space-y-3">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3">
            <Image src={item.image} alt={item.name} width={48} height={48} className="w-12 h-12 rounded-lg object-cover flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 truncate">{item.name}</p>
              <p className="text-xs text-gray-500">1 × ${item.price.toLocaleString()}</p>
            </div>
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-sm font-bold text-primary-700">${getSubtotal().toLocaleString()}</span>
        </div>
